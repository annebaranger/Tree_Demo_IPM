...
shade=read.csv2("data/Traits/data_Niinemets&Valladares_2006.csv") |>
  rename(species=Species) |>
  filter(species %in% sp_list) |>
  mutate(shade_tolerance.mean=as.numeric(shade_tolerance.mean)) |>
  dplyr::select(species,shade_tolerance.mean)

try_data<-read.csv2("data/Traits/try/try_query.csv")
...
wooddensity<-read.csv2("data/Traits/GlobalWoodDensityDatabase.csv") %>%
